...
export function useAudioTranscription() {
  ...
  const [contentQuality, setContentQuality] = useState<{ score: number; reason: string } | null>(null)
  ...
  const generateTopicalNote = async (text: string): Promise<string> => {
    ...
    try {
      ...
      if (!analysis.isNoteworthy) {
        console.log(`Content filtered out: ${analysis.reason} (score: ${analysis.score.toFixed(2)})`)
...
Transcription: "${text}"`,
      ...
      const isDuplicate = Array.from(processedSummariesRef.current).some((existingSummary) => {
        // Simple string similarity check
        const similarity = calculateSimpleStringSimilarity(summaryText.trim(), existingSummary)
  ...
  const calculateSimpleStringSimilarity = (str1: string, str2: string): number => {
  ...
  const checkForSpeechPause = () => {
    ...
    if (wordCount >= maxContentLength && timeSinceLastProcessing > minTimeBetweenNotes) {
    ...
    const isCurrentlySilent = recentLevels.every((level) => level < minSilenceLevel)
    ...
    if (isCurrentlySilent) {
      if (silenceStartTimeRef.current === null) {
        ...
        if (wordCount >= minContentLength && timeSinceLastProcessing > minTimeBetweenNotes) {
  ...
  const processAudioChunks = async () => {
    if (audioChunksRef.current.length > 0) {
      ...
      try {
        ...
        console.log(`Audio blob size: ${audioBlob.size} bytes, MIME type: ${mimeType}`)
        ...
        try {
          ...
          console.log(`Audio data: ${uint8Array.length} bytes, first few bytes:`, uint8Array.slice(0, 20).join(", "))
          ...
          if (result && result.text) {
            ...
            console.log("Accumulated content:", accumulatedTranscriptionRef.current)
            ...
            const now = Date.now()
            const timeSinceLastProcessing = now - lastProcessingTimeRef.current
            if (timeSinceLastProcessing > 3000 && accumulatedTranscriptionRef.current.trim().length > 0) {
  ...
  const handleTranscriptionError = (error: Error) => {
    ...
    if (transcriptionErrorCountRef.current >= maxTranscriptionErrors) {
      ...
      console.log("Added fallback message due to repeated transcription errors")
    }
  }
  ...
  const stopRecording = useCallback(async () => {
    ...
    // Log statistics
    console.log(`Content filtering stats: ${filteredContentCountRef.current} segments filtered out`)
  }, [])
